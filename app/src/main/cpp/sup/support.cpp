...
int Freqarrary[] = {17500, 17850, 18200, 18550, 18900, 19250, 19600, 19950, 20300,
                    20650};    //设置播放频率
...
double demo(short *CoRecord, int now, Last *last,
            LEVD *levdI, LEVD *levdQ, double DIST[110]) {
    double totPhase = 0;
    double *Phase[numfre];
    double *Dist[numfre];
    bool flag = true;
    ...
    for (int w = 0; w < numfre; w++) {
        ...
        for (int i = 55; i < 165; i++) {
            tempII[i - 55] = (double) II[i];
            tempQQ[i - 55] = (double) QQ[i];
        }
        //----------------LEDV-----------------
...
//            //LOGW("%lf",Dist[w][110-1]);
            if (fabs(Dist[w][IQ_Length - 1] - last[w].lastDist) < 0.2 ||
                fabs(Dist[w][IQ_Length - 1] - last[w].lastDist) > 10.0 ||
                fabs(Dist[w][IQ_Length - 1]) > 100.0)
                flag = false;        //过滤
